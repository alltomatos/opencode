...
- **RF-18**: Nessa aba, o usuário define independentemente quatro campos — **decidido (2026-08-20)**: **provedor** (ex.: Omniroute), **modelo de áudio** (a conversa em si — ex.: `openai/gpt-audio-mini`), **modelo de transcrição** (STT — ex.: `whisper-1`, usado se a arquitetura escolhida na seção 8 precisar de STT separado), e **modelo de memória** (campo separado dos outros três, confirmado — resumir é tarefa de texto puro, compensa usar algo mais barato que o modelo de áudio da conversa).
...
**Retenção do arquivo temporário — decidido (2026-08-20):** mantém por **7 dias** depois que o resumo é gerado com sucesso, e então apaga automaticamente — meio-termo entre "apaga na hora" (perde o registro bruto se o resumo sair ruim e precisar reprocessar) e "mantém indefinidamente" (superfície de dado sensível parado em disco sem necessidade).
