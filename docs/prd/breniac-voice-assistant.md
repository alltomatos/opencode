...
### 5.8 Configurações
- **RF-17**: O Breniac tem uma aba própria nas Configurações do opencode (mesmo padrão de abas já usado hoje — Geral/Provedores/Servidores/Modelos/MCP/Skill), separada da configuração de modelo usada pelas sessões de chat normais.
- **RF-18**: Nessa aba, o usuário define independentemente: **provedor** (ex.: Omniroute), **modelo de áudio** (a conversa em si — ex.: `openai/gpt-audio-mini`), **modelo de transcrição** (STT — ex.: `whisper-1`, usado se a arquitetura escolhida na seção 8 precisar de STT separado), e **modelo de memória** (resumo de texto ao final da sessão, seção 8.5 — proposta do Claude, 2026-08-20: separado dos outros três porque resumir é tarefa de texto puro, tipicamente compensa usar algo mais barato que o modelo de áudio da conversa; a ser confirmado).
- **RF-19**: Nenhum desses quatro campos tem valor obrigatório pré-definido que force um provedor específico — segue o mesmo princípio de configuração explícita já usado pro resto do opencode (nada hardcoded pro Omniroute só porque foi o que validamos primeiro).
...
- **RNF-03 (privacidade)**: áudio do microfone é sensível por natureza. Precisa ficar claro pro usuário quando está sendo capturado (RF-02) e o áudio deve ir só pro provedor configurado (RF-18), sem gravação/persistência desnecessária local ou no servidor além do necessário pro turno atual. O arquivo temporário de transcrição da conversa (seção 8.5) guarda texto, não áudio, e expira automaticamente (proposta: 7 dias) depois que o resumo é gerado com sucesso.
...
**Captura incremental e arquivo temporário — decidido (2026-08-20, proposta de Ronaldo):** o passo de resumo não depende de manter a sessão inteira em memória de processo até o final. Enquanto a conversa acontece, cada turno (fala transcrita do usuário + resposta em texto do Breniac) é **anexado incrementalmente** a um arquivo temporário da conversa (ex.: `~/.local/share/opencode/breniac/tmp/<id-da-sessao-de-voz>.md`), não só acumulado na memória do app. Ao desligar o Breniac (gatilho já definido acima), o **modelo de memória** (RF-18) lê esse arquivo — não um objeto em memória — e gera o resumo do dia.

Vantagens sobre manter só em memória de processo: (a) se o app cair no meio de uma conversa longa, o que já foi dito não se perde; (b) o passo de resumo vira uma operação simples de ler-um-arquivo-e-chamar-um-modelo, testável isoladamente sem precisar recriar o estado de uma sessão de voz inteira.

**Em aberto**: o que fazer com o arquivo temporário depois que o resumo é gerado com sucesso — apagar imediatamente (mais privado, mas perde o registro bruto se o resumo tiver saído ruim e precisar reprocessar) ou manter por alguns dias como backup do resumo antes de expirar (mais seguro pra depurar, mas é mais uma superfície de dado sensível parado em disco). Tendo a favor de manter por um período curto (proponho 7 dias) e depois apagar — meio-termo entre os dois riscos.
...
9. Confirmar com o Ronaldo se "modelo de memória" (RF-18) é mesmo um quarto campo separado, como proposto, ou se deveria reaproveitar um dos outros três.
10. Implementar a aba de Configurações do Breniac (RF-17/18/19) e o mecanismo de captura incremental em arquivo temporário (seção 8.5) — esses dois são pré-requisitos de infraestrutura pra qualquer protótipo funcional, não só o modelo de voz em si.
