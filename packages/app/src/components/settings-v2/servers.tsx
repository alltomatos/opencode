...
export const SettingsServersV2: Component = () => {
  ...
  const showSearch = createMemo(
    () =>
      controller.sortedItems().filter((item) => !isWslServer(item) && !isSshTunnelServer(item)).length +
        wslServers().length +
        sshServers().length >
      1,
  )
  ...
  const filtered = createMemo(() => {
    const items = controller.sortedItems().filter((item) => !isWslServer(item) && !isSshTunnelServer(item))
